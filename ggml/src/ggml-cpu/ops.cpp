...
// @andrei added 3D RoPE implementation
// static void ggml_compute_forward_rope_3d_f32(
//     const struct ggml_compute_params * params,
//     struct ggml_tensor * dst) {

//     // printf("=== ROPE_3D DEBUG ===\n");
//     // printf("dst: %p\n", dst);
//     // printf("src[0]: %p\n", dst->src[0]);
//     // printf("src[1]: %p\n", dst->src[1]);  
//     // printf("src[2]: %p\n", dst->src[2]);
    
//     if (!dst->src[0] || !dst->src[1] || !dst->src[2]) {
//         printf("ERROR: NULL source tensor!\n");
//         return;
//     }
    
//     // printf("src[0] shape: [%lld, %lld, %lld]\n", dst->src[0]->ne[0], dst->src[0]->ne[1], dst->src[0]->ne[2]);
//     // printf("src[1] shape: [%lld, %lld]\n", dst->src[1]->ne[0], dst->src[1]->ne[1]);
//     // printf("src[2] shape: [%lld, %lld]\n", dst->src[2]->ne[0], dst->src[2]->ne[1]);
...
//     const int d_head = src0->ne[0];
//     const int n_heads = src0->ne[1];     
//     const int n_patches = src0->ne[2];
//     const int d_pair = d_head / 2;
...
//     const int patches_per_thread = (n_patches + nth - 1) / nth;
//     const int patch_start = patches_per_thread * ith;
//     const int patch_end = (patch_start + patches_per_thread < n_patches) ? 
//                          (patch_start + patches_per_thread) : n_patches;
    
//     float * dst_data = (float *) dst->data;
//     const float * src_data = (const float *) src0->data;
//     const float * cos_ptr = (const float *) src1->data;
//     const float * sin_ptr = (const float *) src2->data;
    
//     for (int patch = patch_start; patch < patch_end; patch++) {
//         for (int head = 0; head < n_heads; head++) {
//             for (int pair = 0; pair < d_pair; pair++) {
//                 const int dim0 = pair;
//                 const int dim1 = pair + d_pair;
                
//                 const size_t src_idx0 = dim0 * nb1/sizeof(float) + head * nb2/sizeof(float) + patch * (nb2 * n_heads)/sizeof(float);
//                 const size_t src_idx1 = dim1 * nb1/sizeof(float) + head * nb2/sizeof(float) + patch * (nb2 * n_heads)/sizeof(float);
                
//                 const size_t cos_idx = pair * n_patches + patch;
                
//                 const float x0 = src_data[src_idx0];
//                 const float x1 = src_data[src_idx1];
//                 const float c = cos_ptr[cos_idx];
//                 const float s = sin_ptr[cos_idx];

//                 // if (patch == 0 && head == 0 && pair == 0 && ith == 0) {
//                 //     printf("RoPE indexing debug:\n");
//                 //     printf("src_idx0=%zu, src_idx1=%zu\n", src_idx0, src_idx1);
//                 //     printf("x0=%.6f, x1=%.6f\n", x0, x1);
//                 //     printf("cos_idx=%zu, cos_val=%.6f, sin_val=%.6f\n", cos_idx, c, s);
//                 //     printf("d_head=%d, n_heads=%d, n_patches=%d\n", d_head, n_heads, n_patches);
//                 //     printf("nb1=%zu, nb2=%zu\n", nb1, nb2);
//                 // }

//                 static bool debug_printed = false;
//                 if (!debug_printed) {
//                     debug_printed = true;
//                     printf("=== SINGLE ROPE_3D DEBUG ===\n");
//                     printf("d_head=%d, n_heads=%d, n_patches=%d\n", d_head, n_heads, n_patches);
//                     printf("nb1=%zu, nb2=%zu\n", nb1, nb2);
                    
//                     // Calculate what the first element indices should be
//                     printf("Expected GGML indexing for [dim0=0, head=0, patch=0]: %d\n", 0);
//                     printf("Expected GGML indexing for [dim1=%d, head=0, patch=0]: %d\n", d_pair, d_pair * n_heads * n_patches);
//                     printf("Your formula gives src_idx0 for [0,0,0]: %zu\n", 0 * nb1/sizeof(float) + 0 * nb2/sizeof(float) + 0 * (nb2 * n_heads)/sizeof(float));
//                     printf("Your formula gives src_idx1 for [%d,0,0]: %zu\n", d_pair, d_pair * nb1/sizeof(float) + 0 * nb2/sizeof(float) + 0 * (nb2 * n_heads)/sizeof(float));
//                 }
                
//                 dst_data[src_idx0] = x0 * c - x1 * s;
//                 dst_data[src_idx1] = x0 * s + x1 * c;
//             }
//         }
//     }
// }
...
static void ggml_compute_forward_rope_3d_f32(
    ...
    const struct ggml_tensor * src0 = dst->src[0]; // q tensor
    const struct ggml_tensor * src1 = dst->src[1]; // cos tensor
    const struct ggml_tensor * src2 = dst->src[2]; // sin tensor
    
    const int d_head = src0->ne[0];      // 80
    const int n_heads = src0->ne[1];     // 16
    const int n_patches = src0->ne[2];   // 280
    const int d_pair = d_head / 2;       // 40
    
    // Get tensor strides for Q tensor
    const size_t q_nb0 = src0->nb[0];    // 4
    const size_t q_nb1 = src0->nb[1];    // 320  
    const size_t q_nb2 = src0->nb[2];    // 5120
    
    static int layer_count = 0;
    static bool debug_printed = false;
    if (!debug_printed) {
        debug_printed = true;
        printf("=== ROPE_3D DEBUG LAYER %d ===\n", layer_count);
        printf("d_head=%d, n_heads=%d, n_patches=%d, d_pair=%d\n", d_head, n_heads, n_patches, d_pair);
        printf("Q strides: nb[0]=%zu, nb[1]=%zu, nb[2]=%zu\n", q_nb0, q_nb1, q_nb2);
    }
    layer_count++;
    ...
    if (layer_count <= 1) {
        printf("RoPE called with n_patches=%d, nth=%d\n", n_patches, nth);
        printf("Thread %d: patch_start=%d, patch_end=%d\n", ith, patch_start, patch_end);
    }
    
    for (int patch = patch_start; patch < patch_end; patch++) {
        for (int head = 0; head < n_heads; head++) {
            for (int pair = 0; pair < d_pair; pair++) {
                const int dim0 = pair;           // First half: 0, 1, 2, ..., 39
                const int dim1 = pair + d_pair;  // Second half: 40, 41, 42, ..., 79
                
                // Q tensor indexing using GGML strides: [d_head, n_heads, n_patches]
                const size_t src_idx0 = (dim0 * q_nb0 + head * q_nb1 + patch * q_nb2) / sizeof(float);
                const size_t src_idx1 = (dim1 * q_nb0 + head * q_nb1 + patch * q_nb2) / sizeof(float);
                
                // Cos/sin indexing: data stored as [patch, dim] in your generation
                const size_t cos_idx = patch * d_head + pair;
                ...
                // PyTorch RoPE formula: q * cos + rotate_half(q) * sin
                // dst_data[src_idx0] = x0 * c - x1 * s;
                // dst_data[src_idx1] = x1 * c + x0 * s;
                dst_data[src_idx0] = x0 * c - x1 * s;
                dst_data[src_idx1] = x1 * c + x0 * s;
                
                // Debug first few operations (only thread 0)
                if (patch >= 2 && patch < 4 && head == 0 && pair < 3 && ith == 0 && layer_count <= 3) {
                    printf("LAYER %d - Patch %d, Head %d, Pair %d: x0=%.6f, x1=%.6f, cos=%.6f, sin=%.6f -> new_x0=%.6f, new_x1=%.6f\n",
                           layer_count-1, patch, head, pair, x0, x1, c, s, dst_data[src_idx0], dst_data[src_idx1]);
                }

                if (patch == 19 && head == 15 && pair < 3 && layer_count <= 3) {
                    printf("THREAD %d - LAYER %d - Patch %d, Head %d, Pair %d: x0=%.6f, x1=%.6f, cos=%.6f, sin=%.6f -> new_x0=%.6f, new_x1=%.6f\n",
                        ith, layer_count-1, patch, head, pair, x0, x1, c, s, dst_data[src_idx0], dst_data[src_idx1]);
                }
            }
        }
    }
...
void ggml_compute_forward_rope_3d(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            ggml_compute_forward_rope_3d_f32(params, dst);
            break;
        default:
            GGML_ABORT("Unsupported type for ROPE_3D");
    }
}
